...
void	ClapTrap::takeDamage(unsigned int amount)
{
	if (_hitpts == 0)
	{
		std::cout << this->getType() << " " << _name << " is already dead and cannot die again" << std::endl;
		return ;
	}
	...
	{
		_hitpts -= amount;
		std::cout << this->getType() << " " << _name << " has taken " 
	...
	else
	{
		...
		std::cout << this->getType() << " " << _name << " has taken " 
			<< amount << " of damage and has " << _hitpts
			<< " HP left" << std::endl;
		std::cout << this->getType() << " " << _name << " is dead" << std::endl;
	}
}
...
void	ClapTrap::beRepaired(unsigned int amount)
{
	if (_hitpts == 0)
	{
		std::cout << this->getType() << " " << _name << " is already dead, cannot be repaired" << std::endl;
		return ;
	}
	if (_energypts > 0 && _hitpts > 0)
	{
		_energypts--;
		_hitpts += amount;
		std::cout << this->getType() << " " << _name << " has been repaired for "
			<< amount << " HP. " << _name << " now has " << _hitpts << " HP" << std::endl;
	}
	else if (_energypts == 0)
	{
		std::cout << this->getType() << " " << _name << " has no energy pts left, cannot be repaired" << std::endl;
	}
}
